  ...
  return (
    <header className="border-b border-slate-800 bg-slate-950 h-14 lg:h-16 px-3 lg:px-6 flex items-center justify-between gap-2">
      <div className="flex items-center gap-2 lg:gap-3">
        {children}
        {/* Compact shell: symbol + PathOS wordmark; same two-tier strategy as shared TopBar. */}
        <div className="hidden sm:block">
          <h1 className="flex items-center" style={{ margin: 0 }}>
            <AppBrand
              asset="symbol"
              variant="iconAndText"
              size={32}
              textClassName="text-base lg:text-lg font-bold text-white"
              alt="PathOS"
            />
          </h1>
        </div>
        <div className="sm:hidden flex-shrink-0">
          <AppBrand asset="symbol" variant="iconOnly" size={28} alt="PathOS" />
        </div>
      </div>
