...
export function Sidebar(props: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Brand row: full badge for richer identity; taglines below. Symbol is for top bar only. */}
      <div className="px-3 py-3" style={{ borderBottom: '1px solid var(--p-border)' }}>
        <h2 className="text-base font-semibold" style={{ color: 'var(--p-text)' }}>
          {/* NavLink from useNavLink() is adapter-provided; rule flags it as "created during render". */}
          {/* eslint-disable-next-line react-hooks/static-components */}
          <NavLink href={DASHBOARD} className="flex items-center w-fit outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)] focus-visible:ring-offset-2 rounded">
            <AppBrand asset="full" variant="iconOnly" size={40} className="flex-shrink-0" alt="PathOS" brandingBaseUrl={props.brandingBaseUrl} />
          </NavLink>
        </h2>
        <p className="text-[11px] font-medium mt-1" style={{ color: 'var(--p-text-muted)' }}>Career Intelligence Dashboard</p>
