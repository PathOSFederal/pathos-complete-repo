...
export function DesktopApp() {
  ...
  // Desktop (Vite) uses base: './' so static assets resolve relative to the loaded HTML.
  // Passing BASE_URL lets the shared shell resolve logo paths (e.g. ./branding/appLogoSymbol.png)
  // so they load from apps/desktop/public/branding/ in dev and from the bundle in production.
  // Type assertion: Vite injects import.meta.env at build time; root tsconfig has no vite/client.
  const metaEnv = (import.meta as { env?: { BASE_URL?: string } }).env;
  const brandingBaseUrl =
    metaEnv !== undefined && metaEnv !== null && metaEnv.BASE_URL !== undefined && metaEnv.BASE_URL !== ''
      ? metaEnv.BASE_URL
      : './';
  return (
    <NavigationProvider adapter={adapter} linkComponent={RouterNavLink}>
      <SharedAppShell
        platform="desktop"
        brandingBaseUrl={brandingBaseUrl}
        rightRail={
          <PathAdvisorRail
